...
""" this is for new partial inf. 
    rectagular box based version
    (for new_dyn_verson_2.py)
    based on torch.nn.Conv2d
    padding is at coarse level"""
import torch
...
def create_modules_2(module_defs):
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            print('in create modules, weight shape: ', modules[0].weight.shape)
            if bn:
            ...
            padding_layer = PADLayer(layersize)
    ...
    def forward(self, input_x, RoIs, his_info):
        if RoIs is not None:
            ...
            for R, R_p, his in zip(Rs, R_ps, his_info):
                ...
                input_x[0, :, ymin_2:ymax_2+1, xmin_2:xmin] = his[2]
                input_x[0, :, ymin_2:ymax_2+1, xmax+1:xmax_2+1] = his[3]
        return input_x
...
class Darknet_2(nn.Module):
    ...
    def __init__(self, config_path, img_size=416):
        ...
        self.hyperparams, self.module_list = create_modules_2(self.module_defs)
        self.yolo_layers = [layer[0] for layer in self.module_list if hasattr(layer[0], "metrics")]
        self.img_size = img_size
        self.seen = 0
        self.header_info = np.array([0, 0, 0, self.seen, 0], dtype=np.int32)

    def forward(self, x, input_n1, input_n2, input_n3, input_n4, input_n5, input_n6, input_n7, input_n8, input_n9, input_n10, input_n11, input_n12, input_n13, RoI_0, RoI_1, RoI_2, RoI_3, RoI_4, RoI_5, RoI_6, RoI_7, RoI_8, RoI_9, RoI_10, RoI_11, RoI_12, RoI_13, targets=None):
        img_dim = x.shape[2]
        loss = 0
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if module_def["type"] in ["upsample", "maxpool"]:
                ...
                #if module[0].weight.shape[1] == 192:
                ...
                if _idx1 < 0 or _idx2 < 0:
                    ...
                    kernel_size = int(module_def["size"])
                    pad = (kernel_size - 1) // 2
                    module[0].padding = (pad, pad)
                    x = module(x)
                else:
                    RoIs = locals()['RoI_' + str(_idx1)][1]    # padded
                    if _idx1 == _idx2:
                        n_RoIs = RoIs
                    else:
                        n_RoIs = locals()['RoI_' + str(_idx2)][0]   # next raw roi
                    if RoIs is not None and n_RoIs is not None:
                        next_size = int(module_def["next_size"])
                        n_filters, d_filters, h_filters, w_filters = module[0].weight.shape
                        n_x, d_x, h_x, w_x = x.shape
                        res = torch.zeros(n_x, n_filters, next_size, next_size)
                        stride = int(module_def["stride"])
                        module[0].padding = (0, 0)
